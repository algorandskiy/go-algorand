...
func loadOldHoldings(dbad dbAccountData, delta accountDelta, fetcher fetcher) (dbAccountData, error) {
...
func loadOldParams(dbad dbAccountData, delta accountDelta, fetcher fetcher) (dbAccountData, error) {
		...
		case nil:
			...
			if len(acctDataBuf) > 0 {
				...
				if dbad.pad.ExtendedAssetHolding.Count > 0 {
					...
					dbad, err = loadOldHoldings(dbad, delta, fetcher)
					if err != nil {
						return err
					}
				}
				if dbad.pad.ExtendedAssetParams.Count > 0 {
					_, delta := a.getByIdx(idx)
					dbad, err = loadOldParams(dbad, delta, fetcher)
					if err != nil {
						return err
					}
				}
...
type fetcher func(key int64) (buf []byte, err error)

func makeAssetFetcher(stmt *sql.Stmt) fetcher {
	return func(key int64) (buf []byte, err error) {
		buf, err = loadGroupData(stmt, key)
		return
	}
}
...
func loadHoldings(stmt *sql.Stmt, eah ledgercore.ExtendedAssetHolding) (map[basics.AssetIndex]basics.AssetHolding, ledgercore.ExtendedAssetHolding, error) {
	...
	for gi := range eah.Groups {
		...
		err = groups[gi].Fetch(fetcher, holdings)
		if err != nil {
...
func loadGroupData(stmt *sql.Stmt, key int64) (buf []byte, err error) {
	err = stmt.QueryRow(key).Scan(&buf)
	if err == sql.ErrNoRows {
		return nil, fmt.Errorf("loadGroupData failed to retrive data for key %d", key)
	}
...
func collapseAssetHoldings(qabq, qaed *sql.Stmt, eah ledgercore.ExtendedAssetHolding, updated map[basics.AssetIndex]basics.AssetHolding, deleted []basics.AssetIndex) (assets map[basics.AssetIndex]basics.AssetHolding, err error) {
	// 1. load all
	assets, _, err = loadHoldings(qabq, eah)
	if err != nil {
		return nil, err
	}
	// 2. remove deleted
...
func collapseAssetParams(qabq, qaed *sql.Stmt, eap ledgercore.ExtendedAssetParams, updated map[basics.AssetIndex]basics.AssetParams, deleted []basics.AssetIndex) (assets map[basics.AssetIndex]basics.AssetParams, err error) {
	// 1. load all
	assets, _, err = loadParams(qabq, eap)
	if err != nil {
		return nil, err
	}
	// 2. remove deleted
...
func accountsNewUpdate(qabu, qabq, qaeu, qaei, qaed *sql.Stmt, addr basics.Address, delta accountDelta, genesisProto config.ConsensusParams, updatedAccounts []dbAccountData, updateIdx int) ([]dbAccountData, error) {
	...
	if delta.old.pad.NumAssetHoldings() <= assetsThreshold && len(delta.new.Assets) <= assetsThreshold {
		...
		if newCount < assetsThreshold {
			...
			assets, err := collapseAssetHoldings(qabq, qaed, oldPad.ExtendedAssetHolding, delta.new.Assets, deleted)
			if err != nil {
				return updatedAccounts, err
			}
