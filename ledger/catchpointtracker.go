...
type catchpointTracker struct {
	...
	catchpointControl catchpointControl
}

// initialize initializes the catchpointTracker structure
func (ct *catchpointTracker) initialize(cfg config.Local, dbPathPrefix string, cc catchpointControl) {
	ct.dbDirectory = filepath.Dir(dbPathPrefix)
	ct.catchpointControl = cc
...
func (ct *catchpointTracker) produceCommittingTask(committedRound basics.Round, dbRound basics.Round, dcr *deferredCommitRange) *deferredCommitRange {
	...
	catchpointInterval := ct.catchpointControl.catchpointInterval
	if catchpointInterval > 0 {
		nextCatchpointRound := ((uint64(dcr.oldBase+dcr.lookback) + catchpointInterval) / catchpointInterval) * catchpointInterval

		if nextCatchpointRound < uint64(dcr.oldBase+dcr.lookback)+dcr.offset {
			mostRecentCatchpointRound := (uint64(committedRound) / catchpointInterval) * catchpointInterval
	...
	if dcr.isCatchpointRound && ct.catchpointControl.enableGeneratingCatchpointFiles {
...
func (ct *catchpointTracker) commitRound(ctx context.Context, tx *sql.Tx, dcc *deferredCommitContext) (err error) {
	...
	defer func() {
		if err != nil {
			if dcc.isCatchpointRound && ct.catchpointControl.enableGeneratingCatchpointFiles {
...
func (ct *catchpointTracker) postCommitUnlocked(ctx context.Context, dcc *deferredCommitContext) {
	if dcc.isCatchpointRound && ct.catchpointControl.enableGeneratingCatchpointFiles && dcc.catchpointLabel != "" {
...
func (ct *catchpointTracker) handleUnorderedCommit(dcc *deferredCommitContext) {
	...
	if ct.catchpointControl.enableGeneratingCatchpointFiles {
...
func (ct *catchpointTracker) isCatchpointRound(offset uint64, dbRound basics.Round, lookback basics.Round) bool {
	if !ct.catchpointControl.forceCatchpointFileWriting {
	...
	return ((offset + uint64(lookback+dbRound)) > 0) &&
		(ct.catchpointControl.catchpointInterval != 0) &&
		((uint64((offset + uint64(lookback+dbRound))) % ct.catchpointControl.catchpointInterval) == 0)
}
...
func (ct *catchpointTracker) catchpointEnabled() bool {
	return ct.catchpointControl.catchpointInterval != 0
}
