...
func makeCatchpointControl(cfg config.Local) (cc catchpointControl) {
	cc.enableGeneratingCatchpointFiles = cfg.Archival
	...
	case forceCatchpointFileGenerationTrackingMode:
		...
		cc.forceCatchpointFileWriting = true
		fallthrough
	case 2:
		cc.catchpointInterval = cfg.CatchpointInterval
		cc.enableGeneratingCatchpointFiles = true
	case 1:
		cc.catchpointInterval = cfg.CatchpointInterval
	default:
		...
		fallthrough
	case 0:
		if cc.enableGeneratingCatchpointFiles {
			cc.catchpointInterval = cfg.CatchpointInterval
		} else {
			cc.catchpointInterval = 0
		}
	case -1:
		cc.catchpointInterval = 0
	}
...
func (tr *trackerRegistry) initialize(l ledgerForTracker, trackers []ledgerTracker, cfg config.Local) (err error) {
	...
	tr.catchpointControl = makeCatchpointControl(cfg)
...
func (tr *trackerRegistry) commitRound(dcc *deferredCommitContext) {
	...
	for _, lt := range tr.trackers {
		...
		if err != nil {
			...
			if dcc.isCatchpointRound && tr.catchpointControl.enableGeneratingCatchpointFiles {
...
func (tr *trackerRegistry) initializeTrackerCaches(l ledgerForTracker) (err error) {
	...
	catchpointInterval := tr.catchpointControl.catchpointInterval
